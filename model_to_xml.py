"""Convert an XGBoost .model file into a TMVA-compatible XML file."""
...
def load_training_variables(info_path):
    with open(info_path, "rb") as handle:
        training_info = pickle.load(handle)

    if "training_variables" not in training_info:
        raise KeyError(f"{info_path} does not contain a 'training_variables' entry")

    return training_info["training_variables"]


def main():
    parser = argparse.ArgumentParser(description="Convert an XGBoost .model file to a TMVA-style XML file.")
    parser.add_argument("model_path", type=Path, help="Path to the input XGBoost .model file")
    parser.add_argument(
        "-i",
        "--info-path",
        type=Path,
        default=None,
        help="Optional path to the matching *_info.pkl file. Defaults to <model_path>_info.pkl.",
    )
    parser.add_argument(
        "-o",
        "--output-xml",
        type=Path,
        default=None,
        help="Output XML path. Defaults to the model path with .xml extension.",
    )
    parser.add_argument(
        "--feature",
        dest="features",
        action="append",
        help="Feature name override. Repeat this option once per feature in training order.",
    )
    ...
    if info_path is None:
        info_path = args.model_path.with_name(args.model_path.stem + "_info.pkl")

    output_xml = args.output_xml
    if output_xml is None:
        output_xml = args.model_path.with_suffix(".xml")

    if args.features is not None:
        training_variables = args.features
    else:
        training_variables = load_training_variables(info_path)

    booster = xgb.Booster()
    booster.load_model(str(args.model_path))
    booster.feature_names = list(training_variables)

    model_dump = booster.get_dump()
    input_variables = [(feature_name, "D") for feature_name in training_variables]
    convert_model(model_dump, input_variables=input_variables, output_xml=str(output_xml))
