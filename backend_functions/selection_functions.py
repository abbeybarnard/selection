...
def bdt_raw_results(train, test, train_query, test_query, varlist, params, rounds):
    ...
    # Track training progress with watchlist and evals_result
    watchlist = [(dtrain, 'train'), (dtest, 'test')]
    evals_result = {}
    
    model = xgb.train(params, dtrain, rounds, evals=watchlist, evals_result=evals_result, verbose_eval=False)
    ...
    # Convert evals_result to DataFrame for plotting
    import pandas as pd
    metrics_df = pd.DataFrame({
        'train-error': evals_result['train']['error'],
        'test-error': evals_result['test']['error'],
        'train-auc': evals_result['train']['auc'],
        'test-auc': evals_result['test']['auc'],
        'train-aucpr': evals_result['train']['aucpr'],
        'test-aucpr': evals_result['test']['aucpr']
    })
    
    return queried_test_df, model, metrics_df
########################################################################
def main_BDT(datasets, train_query, test_query, rounds, training_parameters, isrun3, test_size=0.5, USE_EXT_IN_BDT=True):
    ...
    queried_test_df, bdt_model, metrics_df = bdt_raw_results(df_pre_train, df_pre_test, train_query, test_query, training_parameters, params, rounds)
    
    d = {
        'bdt_results_df': metrics_df,  # DataFrame with training metrics (error, auc, aucpr) 
        'queried_test_df': queried_test_df,  # Test predictions
        'bdt_model': bdt_model, 
...
def bdt_pe(df, xvals, gen_data, gen_intrinsic, split=1.0, test_size=None):
    ...
    # Accept either the historic `split` positional arg or the newer `test_size` keyword
    if test_size is not None:
        split = test_size
    else:
        # If user passed `gen_data` from the full dataset but `df` is only the test set,
        # attempt to auto-compute the split fraction based on summed signal weights
        # so weighted generated counts are used for efficiency calculations.
        try:
            sig_weights_in_df = np.nansum(df.query('is_signal == True').weight)
            total_gen_weights = np.nansum(gen_data) if len(gen_data) > 0 else 0.0
            if total_gen_weights > 0:
                split = float(sig_weights_in_df) / float(total_gen_weights)
        except Exception:
            # If any issue occurs (e.g., gen_data not array-like), fall back to provided split
            pass
    ...
    for cut_val in xvals:
        ...
        # total signal generated (scale by split fraction)
        tot_sig = np.nansum(gen_data) * split if len(gen_data) > 0 else 0.0
        tot_sig_intrinsic = np.nansum(gen_intrinsic) * split if len(gen_intrinsic) > 0 else 0.0

        # Purity: protect against division by zero
        if tot_sel == 0:
            purity.append(0.0)
            purErr.append(0.0)
        else:
        ...
        # Efficiency: protect against zero generated signal
        if tot_sig == 0 or tot_sig_intrinsic == 0:
            eff.append(0.0)
            effErr.append(0.0)
        else:
...
def bdt_box_plot(results_bdt, xvals, isrun3, second_results_bdt=None, results_box=None, results_box_err=None, save=False, 
                save_label=None, title=None, target_purity=None):
    ...
    pur = np.array(pur)
    eff = np.array(eff)
    purErr = np.array(purErr)
    effErr = np.array(effErr)
    ...
    # If a target purity is specified, find the closest BDT cut and annotate the plot
    if target_purity is not None:
        try:
            # find index with purity closest to target
            idx = int(np.argmin(np.abs(pur - float(target_purity))))
            cut_val = xvals[idx]

            # Draw vertical line at the cut
            plt.axvline(cut_val, color='gray', linestyle='--', linewidth=1)

            # Draw horizontal line at target purity
            plt.axhline(target_purity, color='maroon', linestyle=':', linewidth=1)

            # Use circle markers for intersections
            plt.plot(cut_val, pur[idx], marker='o', color='maroon', markersize=8, markeredgecolor='darkred', markeredgewidth=0.8)
            plt.plot(cut_val, eff[idx], marker='o', color='green', markersize=8, markeredgecolor='darkgreen', markeredgewidth=0.8)

            # Compute offsets so annotations sit a bit to the left (purity) and right (efficiency) and above the markers
            xvals_arr = np.array(xvals)
            x_range = float(xvals_arr[-1] - xvals_arr[0]) if xvals_arr.size > 1 else 0.1
            x_off = x_range * 0.04
            y_off = 6.0

            # Annotate with only the numeric values (no labels).
            # Purity on the left, efficiency on the right.
            plt.text(cut_val - x_off, pur[idx] + y_off, f'{pur[idx]:.1f}%', color='maroon', horizontalalignment='right', fontsize=10)
            plt.text(cut_val + x_off, eff[idx] + y_off, f'{eff[idx]:.1f}%', color='green', horizontalalignment='left', fontsize=10)

            # Print the cut and associated metrics to stdout
            print(f"Target purity {target_purity}% -> BDT cut > {cut_val:.3f}")
            print(f"  BDT Purity: {pur[idx]:.2f}% ± {purErr[idx]:.2f}%")
            print(f"  BDT Efficiency: {eff[idx]:.2f}% ± {effErr[idx]:.2f}%")
        except Exception:
            pass
